...
### Editorial architecture (founding rule)

RADAR is a **digital newsroom**, not an aggregator. The founding product manifesto
is **[`RADAR_PRODUCT_ARCHITECTURE.md`](RADAR_PRODUCT_ARCHITECTURE.md)** — read it
first. Every content item is processed **once** by a single **editorial pipeline**
(Collecte → Qualification → Lecture IA → Qualification éditoriale → Sujets →
Briefing); screens are only **views** of that shared intelligence. Business objects:
Source · Contenu · Sujet · Récit · Briefing · Vue. **The pipeline decides, screens
present** — no business logic in React components. Freshness is a property of the
content, never of the screen: never recompute freshness, themes, relevance or
categories per screen — consume the common qualification
(`src/lib/qualificationContenu.ts`). The unit of reading is the **Sujet** (the
article is a proof), not the article. Every feature must answer one question:
*"which stage of the editorial pipeline does it belong to?"* — if none, it
