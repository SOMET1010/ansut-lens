...
> principes : il décrit **les clics à faire, par qui, dans quel ordre**.
>
...
C'est le vrai point bloquant, et il faut le dire précisément : il n'y a **pas un
seul** compte admin, il y en a **trois différents**, sur trois plateformes, et
...
**Conséquence pratique** : ce n'est pas « un » admin qu'il faut trouver, c'est
**identifier nommément trois personnes** (ou une seule qui cumule les trois
rôles). Première action concrète, avant toute technique :
...
C'est le **plus rapide** et celui qui débloque le plus de valeur. À faire en
premier.
...
Aucune des trois procédures ci-dessus ne couvre les commentaires sous un post
d'un **média tiers**. C'est précisément le cas qui a échappé à RADAR le 17/08.
Trois options, à arbitrer par la Direction :

| Option | Ce que ça donne | Coût | Délai |
|---|---|---|---|
| **A. Signalement manuel assisté** | Un agent COM colle l'URL du post dans RADAR ; RADAR crée un fil à suivre et alerte sur la tonalité des commentaires saisis | **0 F** | implémentable **immédiatement**, sans aucun accès externe |
| **B. Prestataire de social listening** (Brand24, Talkwalker, Meltwater, Mention) | Couverture réelle des commentaires publics, y compris chez les tiers, via leurs licences Meta | 150–800 USD/mois | 1–2 semaines |
| **C. Surveillance des pages médias** (RSS + volume de partages) | Signal faible : on voit qu'un article circule, pas ce qui s'y dit | 0 F | 1 itération |
...
## 6. Récapitulatif — qui fait quoi, dans quel ordre

| # | Action | Responsable | Durée | Bloque quoi |
|---|---|---|---|---|
| 0 | Lister nominativement les 3 admins (FB / X / LinkedIn) | DIRCOM | 1 jour | tout le reste |
| 1 | Créer `social@ansut.ci` + l'ajouter comme admin partout | DSI + DIRCOM | 1 jour | pérennité des accès |
| 2 | Souscrire plan X Basic + régénérer le Bearer Token | COM + achat | 1 jour | veille X et réponses |
| 3 | Créer l'app Meta + demander les permissions | admin Page FB | 1 j + 3–10 j Meta | commentaires Page ANSUT |
| 4 | Réactiver l'app LinkedIn + demander la Community API | admin Page LI | 1 j + 2–4 sem. | commentaires LinkedIn |
| 5 | Arbitrer l'option posts tiers (A / B / C) | Direction | décision | angle mort SIKA Finance |
| 6 | Implémenter le signalement manuel + règle d'alerte | RADAR | 1 itération | **rien — faisable tout de suite** |
