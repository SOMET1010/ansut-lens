    ...
    return (
      <Card className="glass">
        <CardContent className="py-12 text-center">
          <Sparkles className="h-12 w-12 mx-auto text-primary/50 mb-4" aria-hidden />
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 mb-2">
        ...
        <Badge variant="secondary" className="text-xs" aria-label={`${recommendations.length} recommandations`}>
          ...
          <CardContent className="py-3 px-4">
            <div className="flex items-start gap-3">
              <div className="flex flex-col items-center gap-1 pt-1">
                <div 
                  className={`text-xs font-bold rounded-full h-8 w-8 flex items-center justify-center ${
                    item.score_personnalise >= 70
                      ? 'bg-[hsl(var(--signal-positive))] text-primary-foreground'
                      : item.score_personnalise >= 50
                      ? 'bg-[hsl(var(--signal-warning))] text-primary-foreground'
                      : 'bg-muted text-muted-foreground'
                  }`}
                  aria-label={`Score de pertinence : ${item.score_personnalise}`}
                  title={`Score de pertinence : ${item.score_personnalise}`}
                >
