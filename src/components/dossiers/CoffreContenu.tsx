...
import { Plus, Copy, Trash2, FileText, Linkedin, Hash } from "lucide-react";
...
/**
 * Coffre à contenus — bibliothèque de posts pré-validés par la Communication.
 *
 * Extrait en composant pour vivre dans le PRODUIT (onglet « Coffre » de Publier)
 * autant que dans la console d'administration. C'est une brique éditoriale : sa
 * place est auprès de la production de contenu, pas dans les réglages.
 */
export function CoffreContenu() {
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="flex items-center gap-2 text-lg font-semibold">
            <FileText size={18} /> Coffre à contenus
          </h2>
          <p className="text-sm text-muted-foreground">Bibliothèque de posts pré-validés, prêts à copier-coller.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          {filtered?.map((contenu) => (
            <Card key={contenu.id} className="group transition-colors hover:border-primary/30">
              ...
              <CardContent className="space-y-3">
                <p className="whitespace-pre-wrap rounded-lg bg-muted/50 p-3 text-sm">{contenu.contenu}</p>
