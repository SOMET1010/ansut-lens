...
serve(async (req) => {
  ...
  try {
    ...
    const categories = Array.from(catMap.entries())
      .map(([name, { sum, count }]) => ({
        name, avg: Math.round((sum / count) * 100) / 100, count,
    ...
    const topSources = Array.from(sourceCount.entries()).sort((a, b) => b[1] - a[1]).slice(0, 5);
    ...
    const html = generateDigestHtml({
      ...
      categories, topSources, hasAlert, alertCategories,
...
function generateDigestHtml(data: DigestData): string {
  ...
  const storiesHtml = stories.map((s: any, i: number) => {
    ...
    const sentEmoji = s.sentiment != null ? (s.sentiment > 0.2 ? "😊" : s.sentiment < -0.2 ? "😟" : "😐") : "⏳";
    return `<tr><td style="padding:14px 0;border-bottom:1px solid #e5e7eb;">
      ...
      <a href="${s.source_url || "#"}" style="color:#1e40af;text-decoration:none;font-weight:600;font-size:15px;line-height:1.4;">${i + 1}. ${s.titre}</a>
  ...
  const chartHtml = includeChart ? `
    <div style="margin:20px 0;padding:20px;background:#f8fafc;border-radius:12px;">
      ...
      <div style="display:flex;justify-content:space-between;font-size:12px;color:#6b7280;">
        <span>😊 Positif: ${sentiment.positive}</span><span>😐 Neutre: ${sentiment.neutral}</span><span>😟 Négatif: ${sentiment.negative}</span>
      </div>
      ${categories.length > 0 ? `<div style="margin-top:16px;border-top:1px solid #e5e7eb;padding-top:12px;">
        <p style="font-size:12px;font-weight:700;color:#6b7280;text-transform:uppercase;margin:0 0 8px;">Par catégorie</p>
        ${categories.map(c => `<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;font-size:13px;">
          ...
          <span style="flex:1;color:#374151;">${c.name}</span><span style="color:#6b7280;">(${c.count})</span>
  ...
  const alertHtml = hasAlert ? `<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:8px;padding:14px 16px;margin-bottom:20px;">
    <strong style="color:#dc2626;">🔴 Alerte sentiment</strong>
    <p style="margin:4px 0 0;color:#991b1b;font-size:13px;">${alertCategories.length > 0
      ? `Catégories en alerte : ${alertCategories.map(c => `${c.name} (${c.avg.toFixed(2)})`).join(", ")}`
      : "Plus de 40% des articles ont un sentiment négatif cette semaine."}</p>
  </div>` : "";

  const sourcesHtml = includeSources && topSources.length > 0 ? `<div style="margin:20px 0;padding:16px;background:#f0fdf4;border-radius:8px;">
    <h3 style="margin:0 0 10px;font-size:14px;color:#111827;">🏆 Top Sources</h3>
    ${topSources.map(([name, count]) => `<div style="display:flex;justify-content:space-between;font-size:13px;padding:4px 0;color:#374151;">
...
<body style="margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,'Helvetica Neue',Arial,sans-serif;background:#f3f4f6;">
  <div style="max-width:640px;margin:0 auto;padding:20px;">
    ...
    <div style="background:white;padding:24px;border-radius:0 0 12px 12px;box-shadow:0 2px 8px rgba(0,0,0,0.08);">
      ...
      <p style="color:#9ca3af;font-size:11px;margin:20px 0 0;text-align:center;">Digest hebdomadaire automatique — ANSUT Radar</p>
    </div>
